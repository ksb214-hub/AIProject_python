...
본 프로젝트는 **모듈화된 구조(Modular Architecture)**를 기반으로 설계되어  
크롤링, 서비스 로직, 유틸리티 기능을 분리하여 관리한다.

```mermaid
flowchart TD

A[AIProject] --> B[Crawling]
A --> C[Service]
A --> D[Utils]
A --> E[data]
A --> F[scheduler.py]
